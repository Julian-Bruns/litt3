# Finite nonempty rank-two candidates for the Hermitian lift
...
For any fixed theta characteristic theta these classes correspond to
dormant projective rank-two opers on C, via
W=V tensor (theta tensor tau^2)^-1. There is no additional 2^(2g) factor
after fixing theta. Isomorphism and marking choices are not claimed finite.
...
Literature-based result with normalization checked by /root and bounded
source agent /root/rank_two_frobenius_preimage_sources, 2026-09-06.
[Proof and precise source boundary](../../Proofs/projective_connections/dormant_rank_two_candidates.md).
