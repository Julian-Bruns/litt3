...
Status: `conditional-proof` local calculation. The algebra in this file is exact and
was replayed in the present checkout, but its coefficient formulas come from
missing normal-form note `165`. It neither proves nor disproves the `c=d`
stratum.
...
In the inherited notation, this is `c=d=a`. It is not excluded by the current
entry-one calculation.
...
For clarity, the additional inherited coefficient identity used here is

```text
b33(x)=-N*x+x*(x-1)*(b+x*f1+x^2*l0+3*x^3),
b=2*p-N,
f1=4*p*(alpha+u)+3*s+2*p+3*q+4.
```

Substitution of `s=2a`, `p=a^2`, `N=(1-a)^2`, and `q=4a` gives

```text
b33(a)+a^3*(a-1)^2
 =(a-1)*a^3*(-a*alpha-a*u+a+l0-2).
```

Thus the tangent condition is equivalent to the linear equation

```text
l0 = a*alpha + a*u - a + 2.
```

The coefficient of `l0` is `a^3(a-1)`, nonzero because `a notin {0,1}`.
Therefore the double finite pole edge remains compatible: this tangent
condition solves `l0`; it does not eliminate `c=d`.

## Exact replay

The displayed factorization is reproduced by:
