...
def solve(tensor,chart,output,seconds,memory_gib,precondition=True):
    ...
    if model_path.exists():
        ...
        model=OriginalChart(tensor,chart,field_model=validated['field_model'])
    else:model=OriginalChart(tensor,chart)
    ...
    result=dict(status='not_started',chart=int(chart),source_sha256=model.source_sha256,
        tensor_path=str(model.path),field_model=model.field_model,variables=model.names,
    ...
    if precondition and immediate is None:
        ...
        conditioner=AffinePrecondition(model)
    ...
    if immediate is not None:
        model.verify_unit(immediate)
        result.update(status='verified_polynomial_certificate',
            polynomial_multipliers=[str(h) for h in immediate],
            identity_sum_original_rows_times_multipliers_equals_one_verified=True,
            max_multiplier_degree=max((h.total_degree() for h in immediate if h),default=0),
            groebner_search_needed=False,elapsed_seconds=time.monotonic()-started)
...
if __name__=='__main__':
    ...
    solve(args.tensor,args.chart,args.output,args.seconds,args.memory_gib,not args.raw)
