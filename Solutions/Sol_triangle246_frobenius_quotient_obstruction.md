...
The source-checked [one-cover tame-lifting argument](Sol_triangle344_frobenius_obstruction.md#1-complete-finite-census-and-its-characteristic-five-use)
...
The [Python implementation](../scripts/verify_triangle246_monodromy.py)
...
written [unpruned C++ cross-check](../scripts/verify_triangle246_unpruned.cpp)
...
Apply [quadrangular_genus_two_hecke_obstruction](../Theorems/Thm_quadrangular_genus_two_hecke_obstruction.md).
...
For C_alpha the verified arithmetic input is End0(J(C_alpha)) a quartic
CM field with Rosati-fixed part Q(sqrt21). This cannot contain sqrt5.
...
python3 scripts/verify_triangle246_monodromy.py --native /tmp/triangle246
```
...
This removes only the degree24 backup profile. The changing remainder
is recorded in [the backup state](../Research/BACKUP_CANDIDATE.md).
The main pair is unchanged; its no-cored proof
already works independently of this auxiliary calculation. No new
independent prose audit or formal verification is claimed.
