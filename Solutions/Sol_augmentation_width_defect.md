...
Author proof,2026-09-11. Fresh focused audit PASS by
/root/audit_augmentation_width_defect, same date; no coefficient correction.
Report Research/audits/AUGMENTATION_WIDTH_DEFECT_AUDIT_2026_09_11.md.
This is a one-leg structural theorem, not an arbitrary common-cover
exclusion. Nothing here is Lean verified.

[Statement](../Theorems/Thm_augmentation_width_defect.md).

## Theorem

Let (C,r) be any active admissible pair over bar(F5), genus at least two,
with indigenous defect exactly one. Let T->C be an actual connected
finite etale Galois cover with finite five-group P. Set R=k[P], let J
be its augmentation ideal, and put

    h_i=dim_k J^i/J^(i+1),
    width_2(P)=max_(i>=0)(h_i+h_(i+1)).

Then the actual source defect satisfies

    defect(T)>=width_2(P).

More precisely D_T=coker(Psi_T) has a one-relation presentation
R/(R f), with f in J^2, after choosing compatible linearized source
and target bases. No commutativity or ordinary-Jacobian assumption is
needed. The same lower bound holds on every further actual finite
etale cover of T.

Examples in characteristic five:

* P=C_q, q=5^a>1: lower bound 2.
* P=(C_q)^2: lower bound 2q-1.
* P=(C_q)^3: lower bound (3q^2-1)/2.
* P=the exponent-five Heisenberg group of order125: lower bound25.

The last conclusion also applies whenever the group P of the given
Galois cover has this Heisenberg quotient, by passing to that actual
intermediate and pulling back further.
It does not say every nonabelian five-group has such a quotient.
...
`scripts/check_augmentation_width.py` constructs the ACTUAL Heisenberg
...
`scripts/audit_augmentation_width.py` and
...
The legacy Heisenberg notes concern local wild ramification or
prime-to-five Raynaud representations; neither gives this actual
five-group indigenous-defect bound. This result does not settle
non-Galois neutral descent or exclude an unrestricted common source.
