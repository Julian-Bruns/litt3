...
OUT = args.data
receipt_path = args.receipt or OUT / "assembly_receipt.json"
if receipt_path.exists():
    raise FileExistsError(f"Receipt exists; choose a new --receipt path: {receipt_path}")
...
partition = {
    ...
    "hermitian_hessian_via_symmetry": [(8, (4, 4, 4)), (24, (3, 3, 4))],
...
def v2(value):
    ...
    while value % 2 == 0:
        ...
        value //= 2
    return exponent
two_adic_valuations = [v2(value) for value in values]
assert two_adic_valuations == [4, 4] and prep["curve"]["genus"] == 2
assert prep["arithmetic"]["all_J2_rational"]
integers = PolynomialRing(ZZ, "T")
T = integers.gen()
P = integers(weil)
frobenius24_remainder_mod27 = [int(c % 27) for c in (T**24 - 1) % P]
assert frobenius24_remainder_mod27 == [18, 0, 0, 18]
assert P(-1) % 3 != 0
# The hyperelliptic Frobenius corollary handles all two-powers.
required_orders = [4, 6, 8, 12, 16, 18, 24, 36]
assert all(order & (order - 1) == 0 or 24 % order == 0 or 36 % order == 0
...
result = {"status": "PASS scoped backup cored small-packet assembly",
          ...
          "mixed_two_three_W1_exponent_bound": 144,
          "one_point_orders_covered": required_orders,
