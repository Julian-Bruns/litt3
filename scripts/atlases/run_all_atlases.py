...
from scripts.atlases import atlas_f4 as f4
from scripts.atlases import atlas_eta
...
FILES = ['scripts/atlases/run_all_atlases.py', 'scripts/atlases/atlas_f4.py',
         ...
         'scripts/atlases/atlas_series.py','scripts/atlases/native/atlas_native_rref.py',
         'scripts/atlases/native/atlas_native_rref.cpp','scripts/atlases/native/atlas_native_directions.py',
         'scripts/atlases/atlas_residue_projection.py','scripts/atlases/native/atlas_resources.py',
         'scripts/atlases/atlas_complete_directions.py','scripts/atlases/atlas_direction_kernel.cpp',
         'scripts/atlases/atlas_factored_R_witness.py',
         'scripts/atlases/native/atlas_native_batch.py','scripts/atlases/native/native_original_atlas.sage',
         'scripts/atlases/atlas_coefficient_codec.py',
         'scripts/atlases/native/atlas_native_R_checks.py','scripts/atlases/atlas_R_check.cpp','scripts/atlases/algebra/atlas_field_maps.py',
         'scripts/atlases/native/atlas_native_tensor_input.py','scripts/atlases/native/prepare_native_atlas_input.sage',
         'scripts/atlases/native/atlas_native_roots.py','scripts/atlases/atlas_frobenius_roots.cpp',
         'scripts/atlases/native/prepare_native_atlas_roots.sage','scripts/atlases/native/atlas_native_recovery.py',
         'scripts/atlases/native/atlas_legacy_native_blocks.py','scripts/atlases/native/pack_legacy_native_direction.sage',
         'scripts/atlases/atlas_deck_chart.py','scripts/atlases/check_atlas_tensor_grading.py',
         'scripts/atlases/atlas_original_chart.py','scripts/atlases/atlas_affine_precondition.py',
         'scripts/atlases/native/verify_native_original_atlas.sage']
DONE = f4.COMPLETED
...
class Queue:
    ...
    def prepare(self, job):
        ...
        if not tensor.exists():
            if not self.command(job, 'building', [self.args.sage, str(ROOT/'scripts/atlases/opers/build_oper_atlas.sage'),
        ...
        if not manifest.get('all_32_complete') and not available:
            if not self.command(job, 'exporting', [self.args.sage, str(ROOT/'scripts/atlases/export_rooted_atlas.sage'),
    ...
    def native_batch(self,job):
        ...
        from scripts.atlases.native.atlas_native_batch import TERMINAL
        ...
        cmd=[sys.executable,str(ROOT/'scripts/atlases/native/atlas_native_batch.py'),
    ...
    def solve(self, job):
        ...
        if done == 32:
            ...
            self.save()
            return
        cmd = [sys.executable, str(ROOT/'scripts/atlases/atlas_f4.py'), 'run',
    ...
    def pencil(self, job):
        ...
        for chart in range(31,-1,-1):
            ...
            if result.get('status') == 'bounded_ansatz_no_certificate' or attempts.get(key,{}).get('status') in (
                ...
                return False
            cmd = [self.args.sage,str(ROOT/'scripts/atlases/mixed_atlas_certificate.sage'),
    ...
    def run(self):
        ...
        if getattr(self.args,'recover_legacy_packing',False):
            from scripts.atlases.native.atlas_native_recovery import recover_verified_packing
        ...
        if getattr(self.args,'recover_native_cleanup',False):
            from scripts.atlases.native.atlas_native_recovery import recover_cleanup,APPROVED_CLEANUP_FAILURES
        ...
        if getattr(self.args,'resume_unstarted_native',False):
            from scripts.atlases.native.atlas_native_recovery import resume_unstarted_user_stop
        ...
        if getattr(self.args,'recover_invariant2_telemetry',False):
            ...
            if job['stage']=='needs_attention':
                ...
                if not (job.get('reason')=='exit_1' and log.name=='solving-1788789910184644000.log' and
                        ...
                        'telemetry_max_degree(events)' in (ROOT/'scripts/atlases/atlas_f4.py').read_text()):
