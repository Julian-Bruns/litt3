...
if __name__=='__main__':
    ...
    def add_solvers(twist):
        ...
        for chart in range(4):
            ...
            command=['sage',str(ROOT/('scripts/genus_two/backup_genus_two_solve.sage' if args.solver_backend=='original' else 'scripts/atlases/native/backup_genus_two_native_solve.sage')),
    ...
    if args.mode=='finish':
        ...
        for twist in [1,2,3]:
            for precision in [500,600]:
                ...
                if not path.exists():raise SystemExit(f'Prepare inventory first: sage scripts/genus_two/backup_genus_two_finish.py --state {state} --inventory')
        ...
        for index in range(max((m['relation_count'] for m in manifests),default=0)):
            for manifest in manifests:
                ...
                add(name,['sage',str(ROOT/'scripts/genus_two/backup_genus_two_finish.py'),
    ...
    def add_finalizers():
        for manifest in manifests:
            ...
            if ready and not tensor_done(Path(manifest['target'])):
                add(Path(state).name+'_aggregate',['sage',str(ROOT/'scripts/genus_two/backup_genus_two_finish.py'),
    ...
    if args.mode in ['tensors','pipeline']:
        for twist in [-1,0,1,2,3]:
            for precision in [500,600]:
                ...
                add(name,['sage',str(ROOT/'scripts/genus_two/backup_genus_two_tensor.sage'),
