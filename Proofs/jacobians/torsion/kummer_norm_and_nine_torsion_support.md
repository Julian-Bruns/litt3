...
Canonical [statement](../../../Theorems/jacobians/torsion/kummer_norm_and_nine_torsion_support.md).
...
The current [Sage certificate generator](../../../scripts/arithmetic/check_fixed_x_double_support_torsion.sage)
...
On U, representatives are x-reduced and avoid all ramification points.
The [general pencil lemma](reduced_divisor_rigidity.md#2-a-second-difference-cannot-move-a-low-degree-class)
applies with n=r=3: every function of degree≤6 lies in k(x), since
an independent function has degree at least9. Its cyclic injectivity
statement gives injectivity of lambda=1−rho on U. Its second-difference
statement applies to every gamma=rho^i pi^j: these actions fix O,
commute with rho and have finite point orbits. The representative's
ramification part is zero, so no pointwise ramification condition on
gamma is needed.

This is injectivity on the indicated locus, not a self-map of it.
Images under lambda can leave U and W3, so the argument cannot be
iterated to exclude the remaining three-distinct-point nine-torsion.
