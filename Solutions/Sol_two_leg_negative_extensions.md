...
[Statement](../Theorems/Thm_two_leg_negative_extensions.md).
Author /root,2026-09-09. Sections1--3 passed a focused medium audit,
/root/audit_first_witt_clump. NEW Section4 (version2) subsequently passed
a separately scoped continuation audit by the same auditor. All covers preserve the two
original embedded endpoint fields.
...
injectivity. Every Frobenius pullback remains nonsplit as well: the
Cartier sequence tensored by omega^(-a) has its possible H1 kernel
inside H^0(F_*omega tensor omega^(-a))=H^0(omega^(1-5a))=0,
for every a>=1. Iterate with a=m,5m,... .

The finite-projective-monodromy argument in Section2 of
[joint_tangent_clump_dormancy](Sol_joint_tangent_clump_dormancy.md)
works with omega^m in place of omega. Here are the changes, so this is
not an unstated strengthening of that audited special case. Normalize by
theta_i^(-m), where theta_i²=omega_i; these roots need not match. If the
normalized bundles were strongly semistable, finite-field Frobenius
periodicity would give global projective frames on finite etale covers
after some common Frobenius iterate F^n. Comparing the endpoint frames
on a common proper refinement gives a CONSTANT PGL2 matrix. The two
finite projective monodromy groups generate a finite group over a finite
subfield of k. The distinguished O-line defines a nonconstant projective
coordinate: its two coordinates are basepoint-free sections of the
positive line theta_i^(m*5^n). A constant ratio would trivialize that
positive line. A rational invariant of the finite matrix group therefore
gives a nonconstant function in the ACTUAL intersection k(X) intersect
k(Y), a contradiction. This uses the same maximal unramified field as
the cited proof, not a presumed finite simultaneous Galois closure.
...
[admissible_two_leg_w2_lifts](Sol_admissible_two_leg_w2_lifts.md), before
...
## 4. Genus-two secants and rigidity of every W2-liftable span

This section is a NEW addition after the audit of Sections1--3; it
subsequently passed a separately scoped audit recorded in the same file.
It needs no numerical certificate or bound on Frobenius height.

### The extension hypersurface

Fix a genus-two curve Y, m>=1, and put L=omega_Y^m and M=omega_Y^(m+1).
The projective extension space is

            P H^1(L^-1)=P H^0(M)^*=P^(2m).

For an effective degree-m divisor D, the kernel of
H1(L^-1)->H1(L^-1(D)) has dimension m, by the cohomology sequence and
negativity. By duality it is the annihilator of H0(M(-D)); its
projectivization is the span of D under the M-map, retaining full divisor
schemes. These kernels form a rank-m bundle over the proper Sym^m(Y).
Its projectivized image S_m is therefore closed, irreducible, and has
dimension at most2m-1.

In fact dim S_m=2m-1. Take a general sum D=P1+...+Pm of distinct points
and nonzero coefficients in its span. Varying the coefficients and points
gives the values and first jets at D. These2m functionals are independent:
M(-2D) has degree2 and h0=1 unless M(-2D)=omega. For general D that
exception does not hold. When m>=2 the Abel map Sym^m(Y)->Pic^m(Y) is
surjective and multiplication by2 is separable; when m=1 the exception
is precisely the finite set of Weierstrass points. Riemann--Roch now
gives jet rank2m. Thus the projective differential of the incidence map
has rank2m-1 at this point. This directly proves the dimension assertion
in characteristic five; no characteristic-zero secant theorem is used.
For m=1 the image is exactly the familiar bicanonical conic.

Therefore S_m is a nonempty hypersurface in P^(2m). Its points have
exactly the following property: the extension E admits a lift to E of
the inclusion L(-D)->L for some effective D of degree m.

### Apply it to a common linear space

Suppose EVERY nonzero class in J_m gives a semistable E on Y. If
dim J_m>=2, its projectivization contains a projective line and so meets
S_m over k. At such a point, L(-D), of degree m, lifts to E. Its
saturation cannot increase its degree, since E is semistable of slope m.
Thus E is an extension of two degree-m lines. After twisting by
theta^(-m), all Frobenius pullbacks are extensions of degree-zero lines,
so E is normalized strongly semistable. Section1's two-leg projective-
monodromy argument forbids this. Hence dim J_m<=1.

If the unique endpoint clump has r>=m points, the semistability hypothesis
is automatic. An unstable class would have a maximal line N of degree
>m; projection N->L would have a nonempty divisor of degree< m.
Section1 at index zero says its support is an endpoint clump. Uniqueness
of the clump makes its support the r given points, which is impossible.
This proves the displayed weighted bound.
