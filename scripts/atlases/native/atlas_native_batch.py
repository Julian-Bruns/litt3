...
import subprocess
import time
from scripts.atlases.native.atlas_native_tensor_input import sha,cache_path
...
def run(args):
    ...
    if getattr(args,'legacy_native_cache',False) and any(phase=='solve' for _,phase in pending) and (source.parent/'oper.sobj').exists() and all(
        ...
        subprocess.run([__import__('sys').executable,str(ROOT/'scripts/atlases/native/atlas_legacy_native_blocks.py'),
    ...
    if any(phase=='solve' for _,phase in pending) and all(
            (bindings/('binding-%02d.json'%i)).exists() for i in range(32)):
        subprocess.run([args.sage,str(ROOT/'scripts/atlases/native/prepare_native_atlas_input.sage'),
                        '--tensor',str(source)],cwd=ROOT,check=True)
        subprocess.run([args.sage,str(ROOT/'scripts/atlases/native/prepare_native_atlas_roots.sage'),
    ...
    if getattr(args,'deck_descended',False) and prepared and prepared.get('all_native_fifth_roots_verified'):
        from scripts.atlases.check_atlas_tensor_grading import grading,GradingUnavailable
    ...
    def launch(chart,phase):
        ...
        if phase=='solve':
            cmd=[args.sage,str(ROOT/'scripts/atlases/native/native_original_atlas.sage'),
        ...
        else:cmd=[args.sage,str(ROOT/'scripts/atlases/native/verify_native_original_atlas.sage'),str(directory/'result.json')]
