...
import time
from pathlib import Path
from scripts.atlases.atlas_series import coefficient as series_coefficient, coefficients as series_coefficients
from scripts.atlases.atlas_series import transport as transport_series, from_terms as series_from_terms
from scripts.atlases.native.atlas_resources import fork_workers,release_scratch,resident_rss
...
def build_oper_atlas(rep, output, verify_full=False, workers=1, memory_gib=8,direction_backend='auto',refresh_preparation=False):
    ...
    fingerprint = hashlib.sha256(b''.join((ROOT/name).read_bytes() for name in
        ['scripts/atlases/opers/build_oper_atlas.sage','scripts/atlases/opers/oper_representatives.sage',
         'scripts/atlases/atlas_series.py','scripts/atlases/native/atlas_resources.py','scripts/atlases/native/atlas_native_directions.py',
         ...
         'scripts/atlases/native/atlas_native_rref.py','scripts/atlases/native/atlas_native_rref.cpp','scripts/atlases/atlas_residue_projection.py'])).hexdigest()
    ...
    if direction_backend!='laurent':
        from scripts.atlases.atlas_complete_directions import CompleteNativeDirections
    ...
    if verify_full:
        if native_directions is not None:
            from scripts.atlases.atlas_factored_R_witness import FactoredRWitness
            def full_R():
