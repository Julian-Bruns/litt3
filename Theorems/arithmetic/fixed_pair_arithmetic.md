...
No ordinarity or 5-rank assertion for \(J(X)\) is included.

With \(\theta=dx/y^2\), one has
\(\operatorname{ord}_O(x)=-3\), \(\operatorname{ord}_O(y)=-10\), and
\(\operatorname{div}(\theta)=16O\).

[Proof and exact arithmetic inputs](../../Solutions/arithmetic/fixed_pair_arithmetic.md).

Version2; the2026-09-13 proof refinement preserves the statement.
Arithmetic inputs have author-prose verification; the two shorter
simplicity arguments passed a bounded independent audit.
