...
2026-09-11. [Statement](../Theorems/Thm_backup_degree_two_prym_reduction.md).
...
The [binary checker](../scripts/check_degree2_frobenius_orbits.py) verifies
...
map. No complete Prym sieve or actual-map comparison has been executed.

## Next finite task and current limitation

Construct these covers or their Prym factors efficiently and obtain a
geometric factor obstruction against the fixed backup Jacobian.
The number1533 is small compared with262143double labels, but it is
not yet an algorithm producing equations or zeta functions. The marked
Frobenius periods concern labels on the fixed X, not moduli periods
of X', U or R. Nothing here rules out a J(B) factor in an individual
Prym. In particular this reduction has not finished degree2.

Inputs: [fixed-X arithmetic](../Solutions/Sol_fixed_pair_arithmetic.md),
[backup arithmetic and portability](../Research/BACKUP_CANDIDATE.md),
[canonical backup simplicity input](../Theorems/Thm_quadrangular_genus_two_hecke_obstruction.md),
and the actual same-source orbifold base-change diagram.
