    ...
    for line in raw.splitlines():
        ...
        if int(pg)==pgid and not status.startswith('Z'):
            if int(uid)!=os.getuid():
                raise PermissionError('Owned job group contains a different UID: '+str(pgid))
...
def run(args):
    ...
    started=time.monotonic();active={};requested=[];next_event=0;peak_cpu=0;peak_rss=0
    ...
    def save(status):
        atomic(marker,dict(schema=1,source_sha256=digest,tensor=str(source),status=status,
            ...
            active={str(c):dict(pid=v['proc'].pid,phase=v['phase']) for c,v in active.items()},
    ...
    def launch(chart,phase):
        ...
        if phase=='solve':
            cmd=[args.sage,str(ROOT/'scripts/native_original_atlas.sage'),
                ...
                '--seconds',str(args.seconds),'--memory-gib',str(max(.125,args.rss_gib/workers-.3))]
        ...
        active[chart]=dict(proc=proc,log=log,log_path=Path(log.name),phase=phase,started=time.monotonic())
    ...
    try:
        while pending or active:
            while pending and len(active)<workers and not requested:
                chart,phase=pending.pop(0);launch(chart,phase)
            ...
            for chart,job in list(active.items()):
                ...
                over=rss>args.rss_gib*1024**3/workers
